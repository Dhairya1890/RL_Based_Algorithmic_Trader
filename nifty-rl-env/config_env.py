...
This is the exact DRY fix flagged as missing in Nitesh's code, applied
to this environment so it does not repeat the same mistake.
"""
...
# NOT the yfinance data pulled in Week 1 (that is now retired).
# ----------------------------------------------------------------------
...
# model as numbers.
# NOTE: '%Deliverble' is intentionally excluded here. Its computed form,
# 'Deliverable_Pct' (Deliverable Volume / Volume), lives in
# ENGINEERED_COLUMNS. Including both would duplicate the concept.
RAW_NUMERIC_COLUMNS = [
...
# Sentiment columns (from Dhairya, sub-problem D). Real data is not
